...
if (empty($_SESSION['WkLeitungIteration'])) {
    $_SESSION['WkLeitungIteration'] = 0;
}
...
<style>
.tabelle td{font: bold 14px Verdana;
...
for($k=1;$k<3;$k++){
    ...
    for($z=1;$z<4;$z++){
        ...
        }elseif($dsatz[$HebungAbsolviert]=='Nein'){
            $R_bg='#FF9A9A';
            $ReadonlyString="Readonly";
        }else{
            $R_bg="";
        ...
        echo "<input type=\"text\" id=\"$ArtPlusVIdInput\" name=\"$ArtPlusV\" size=\"2\" value=\"$Hgw\" oninput=\"GenerateSafeButton('".$ArtPlusV."',".$i.")\" $ReadonlyString> ";
        //Funktion "Steigerung_safe" ist von wk_funktionen/steigerung_funktion.php
        echo "<button type=\"button\" id=\"$ArtPlusVId\"  style=\"display: none; \" onclick=\"Steigerung_safe(this.value,'".$Art."',".$V.")\" value=\"$i\">Safe</button>";
        echo "</td>";
