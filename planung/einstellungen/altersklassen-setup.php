...
$i++;
 ...
 echo '<tr>';
 if($LastBis>=$dsatz['Von']) echo "<td><input type=\"text\" name=$Von id=$Von size=\"6\" value=\"".$dsatz['Von']."\" style=\" border: 3px solid red;\"></td>";
 else echo "<td><input type=\"text\" name=$Von id=$Von size=\"6\" value=\"".$dsatz['Von']."\"></td>";

   echo "<td><input type=\"text\" name=$Bis id=$Bis size=\"6\" value=\"".$dsatz['Bis']."\"></td>";

   if($dsatz['Klasse']=='Aktive' || $dsatz['Klasse']=='Junioren') echo "<td><input type=\"text\" name=$Class id=$Class size=\"10\" value=\"".$dsatz['Klasse']."\" readonly></td>";
 ...
 $LastBis=$dsatz['Bis'];
    ...
    if(isset($_POST['speichern'])){
         ...
         if(($xx=="AK")||($xx=="AK1")){
                 $error=$i;
                 echo '<p style="color:red">Error: Ungültiger Name! Seite muss neugeladen werden!</p>';
                 exit();
         }
...
if(isset($_POST['newAlKl-A']) || isset($_POST['newAlKl-S']) || isset($_POST['newAlKl-K']) || isset($_POST['newAlKl-J'])){
    ...
    $RowsNumDB = dbBefehl(" SELECT * FROM alters_klassen_zk_".$data_a_db['S_Db']);
    $RowsCounter=mysqli_num_rows($RowsNumDB) +1;
        ...
        $NeuAlKlGwKl_W=$Neue_AlKl . "_W";
                ...
                dbBefehl("INSERT INTO alters_klassen_zk_".$data_a_db['S_Db']." (Id,Von,Bis,Klasse)
                       Values ('$RowsCounter','-1','0','$Neue_AlKl')");
